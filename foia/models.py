...
_COMPANIES_INSTALLED = any(
    app == 'companies' or app.startswith('companies.')
...
# StatutoryExemption
# ---------------------------------------------------------------------------
class StatutoryExemption(models.Model):
    """Reference table of Connecticut FOIA statutory exemptions."""
    ...
    class Meta:
        ...
        verbose_name = _('Statutory Exemption')
        verbose_name_plural = _('Statutory Exemptions')
...
class FOIARequest(models.Model):
    """A Freedom of Information Act request received by DECD."""
    ...
    assigned_to = models.ForeignKey(
        ...
        null=True, blank=True, related_name='assigned_foia_requests',
    )
    reviewing_attorney = models.ForeignKey(
        settings.AUTH_USER_MODEL, on_delete=models.SET_NULL,
        null=True, blank=True, related_name='reviewing_foia_requests',
    )

    # Tracking
    created_by = models.ForeignKey(
        settings.AUTH_USER_MODEL, on_delete=models.SET_NULL,
        null=True, blank=True, related_name='created_foia_requests',
    )
    created_at = models.DateTimeField(auto_now_add=True)
    ...
    class Meta:
        ...
        verbose_name = _('FOIA Request')
        verbose_name_plural = _('FOIA Requests')
        indexes = [
            models.Index(fields=['status'], name='idx_foia_status'),
            models.Index(fields=['statutory_deadline'], name='idx_foia_deadline'),
            models.Index(fields=['request_number'], name='idx_foia_number'),
        ]
...
# Conditional: link to companies.Company only when Beacon's companies app is installed
if _COMPANIES_INSTALLED:
    FOIARequest.add_to_class(
        'related_companies',
        models.ManyToManyField(
            'companies.Company', blank=True, related_name='foia_requests',
        ),
    )
...
# FOIAScope
# ---------------------------------------------------------------------------
class FOIAScope(models.Model):
    """Defines search parameters for a FOIA request."""

    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    foia_request = models.OneToOneField(
        FOIARequest, on_delete=models.CASCADE, related_name='scope',
    )
    ...
    class Meta:
        verbose_name = _('FOIA Scope')
        verbose_name_plural = _('FOIA Scopes')

    def __str__(self):
        return f"Scope for {self.foia_request.request_number}"


# ---------------------------------------------------------------------------
# FOIASearchResult
# ---------------------------------------------------------------------------
class FOIASearchResult(models.Model):
    ...
    foia_request = models.ForeignKey(
        FOIARequest, on_delete=models.CASCADE, related_name='search_results',
    )
    ...
    # Snapshot of the record at search time
    ...
    class Meta:
        ...
        verbose_name = _('FOIA Search Result')
        verbose_name_plural = _('FOIA Search Results')
...
# FOIADetermination
# ---------------------------------------------------------------------------
class FOIADetermination(models.Model):
    ...
    search_result = models.OneToOneField(
        FOIASearchResult, on_delete=models.CASCADE, related_name='determination',
    )

    decision = models.CharField(
        max_length=20, choices=Decision.choices,
    )
    exemptions_claimed = models.ManyToManyField(
        StatutoryExemption, blank=True, related_name='determinations',
    )
    ...
    reviewed_by = models.ForeignKey(
        settings.AUTH_USER_MODEL, on_delete=models.SET_NULL,
        null=True, blank=True, related_name='foia_determinations',
    )
    ...
    class Meta:
        verbose_name = _('FOIA Determination')
        verbose_name_plural = _('FOIA Determinations')

    def __str__(self):
        return f"Determination: {self.get_decision_display()} - {self.search_result}"


# ---------------------------------------------------------------------------
# FOIAResponsePackage
# ---------------------------------------------------------------------------
class FOIAResponsePackage(models.Model):
    """The compiled response package for a FOIA request."""

    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    foia_request = models.OneToOneField(
        FOIARequest, on_delete=models.CASCADE, related_name='response_package',
    )
    ...
    class Meta:
        verbose_name = _('FOIA Response Package')
        verbose_name_plural = _('FOIA Response Packages')

    def __str__(self):
        return f"Response Package for {self.foia_request.request_number}"


# ---------------------------------------------------------------------------
# FOIAAppeal
# ---------------------------------------------------------------------------
class FOIAAppeal(models.Model):
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    foia_request = models.ForeignKey(
        FOIARequest, on_delete=models.CASCADE, related_name='appeals',
    )
    ...
    class Meta:
        ordering = ['-filed_date']
        verbose_name = _('FOIA Appeal')
        verbose_name_plural = _('FOIA Appeals')
...
# ---------------------------------------------------------------------------
# FOIADocument — searchable document repository
# ---------------------------------------------------------------------------
class FOIADocument(models.Model):
    """A document uploaded for FOIA search.

    In standalone Admiralty mode, this is the primary search target.
    In Beacon mode, this supplements Interaction/Note search with
    ...
    content = models.TextField(
        blank=True,
        help_text=_('Extracted text content for full-text search'),
    )
    file = models.FileField(upload_to='foia_documents/%Y/%m/', blank=True)

    # Metadata
    document_date = models.DateField(
        null=True, blank=True,
        help_text=_('Date of the original document'),
    )
    source = models.CharField(
        max_length=255, blank=True,
        help_text=_('Where this document came from (e.g. department, system)'),
    )
    ...
    uploaded_by = models.ForeignKey(
        settings.AUTH_USER_MODEL, on_delete=models.SET_NULL,
        null=True, blank=True, related_name='foia_documents',
    )
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
    ...
    class Meta:
        ordering = ['-created_at']
        verbose_name = _('FOIA Document')
        verbose_name_plural = _('FOIA Documents')
...
# ---------------------------------------------------------------------------
# FOIARequestStatusHistory — audit trail for workflow transitions
# ---------------------------------------------------------------------------
class FOIARequestStatusHistory(AbstractStatusHistory):
    """Immutable audit trail of FOIA request status transitions."""
    foia_request = models.ForeignKey(
        'FOIARequest', on_delete=models.CASCADE, related_name='status_history',
    )
